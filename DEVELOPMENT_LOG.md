...
# 2026-08-09 选址交接处理状态回传

- 类型：跨项目链路可靠性
- 内容：Site 在领取资料、下载原件、分析和回传结果时更新处理阶段；任一步骤失败都会把具体卡点和原因回传给工作台中转层，同时保留原异常并等待自动重试。
- 验证方法：运行 `python -m unittest tests.test_remote_site_handoff` 与全量测试。

# 2026-08-09 正式交接与只读分析权限分离

- 类型：跨项目交接修复
- 内容：修复负责人确认后资料包被 Site 只读解析器拒绝的问题。外层交接保留“允许进入正式场地跟进”的业务授权，内部 PDF、补充资料和影子分析统一使用只读副本，不继承 Dashboard 写入权限；子步骤失败时回传真实错误原因，不再只显示技术命令。
- 验证方法：使用“上海太古源”真实输入包和 PDF 完整生成影子分析与候选记录；运行 `python -m unittest tests.test_new_store_handoff tests.test_remote_site_handoff -v`。

# 2026-08-09 新店事项一致性校验与可读失败原因
