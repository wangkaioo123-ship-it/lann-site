...
`analysis_catalog.json` 使用 `professional-analysis-catalog/v0.1`，为每家参与门店生成稳定 `analysis_id`，并冻结 canonical 门店、月份、规范化完整输入身份、规则版本、可信度、事实、统计差异、代理指标、假设、缺口、结论与建议。`analysis_id` 直接绑定必要输入的 SHA-256、数据版本和来源提交；输入顺序变化不改变身份，输入内容或版本变化则生成不同身份。缺少该目录的旧成功月份也会进入自动补跑，但旧 run 不覆盖。Dashboard 的人工评审、动作和结果通过独立反馈契约返回，不能覆盖原始分析；详见 `docs/PROFESSIONAL_ANALYSIS_FEEDBACK_V0.1.md`。
